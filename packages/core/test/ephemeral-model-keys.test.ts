...
describe("ephemeral model-key store (device-held seeds)", () => {
  it("stores keys per provider, normalizes ids, and lists metadata without secrets", async () => {
    ...
    expect(listed).toEqual([
      { provider: "anthropic", label: "default", configured: true, updatedAt: expect.any(String), scope: "account" },
      { provider: "openai", label: "default", configured: true, updatedAt: expect.any(String), scope: "account" },
    ]);
    ...
    expect(entries).toEqual(
      expect.arrayContaining([
        { provider: "anthropic", label: "default", key: "sk-ant-123", updatedAt: expect.any(String), scope: "account" },
        { provider: "openai", label: "default", key: "sk-oai-456", updatedAt: expect.any(String), scope: "account" },
      ]),
    );
  });
