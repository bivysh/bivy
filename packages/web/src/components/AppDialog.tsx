...
export function ConfirmDialog({
  ...
  return createPortal(
    <div className="app-dialog" role="dialog" aria-modal="true" aria-labelledby={titleId}>
      <div className="app-dialog-backdrop" onClick={closeWithBack} />
      <div className="app-dialog-body" ref={bodyRef}>
        <h3 id={titleId}>{title}</h3>
        <p>{message}</p>
        <div className="app-dialog-actions">
          <button ref={cancelRef} className="btn" onClick={closeWithBack}>Cancel</button>
          <button className={danger ? "btn danger" : "btn primary"} onClick={() => { closeWithBack(); onConfirm(); }}>{confirmLabel}</button>
        </div>
      </div>
    </div>,
    document.body,
...
export function RenameDialog({
  ...
  const submit = (e: FormEvent) => {
    ...
    if (next && next !== initialValue) { closeWithBack(); onSave(next); }
    else closeWithBack();
  };
  return createPortal(
    <div className="app-dialog" role="dialog" aria-modal="true" aria-labelledby={titleId}>
      ...
      <form ref={bodyRef} className="app-dialog-body" onSubmit={submit}>
        ...
        <div className="app-dialog-actions">
          <button className="btn" type="button" onClick={closeWithBack}>Cancel</button>
