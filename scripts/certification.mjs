...
async function latest() {
  ...
  const report = [`# Latest upstream certification drift`, "", "| Agent | Production pin | Latest | In supported range |", "| --- | --- | --- | --- |", ...rows.map((row) => `| ${row.agent} | ${row.pinned} | ${row.latest} | ${row.inRange ? "yes" : "**no — review required**"} |`), "", "This report is informational and does not alter production support."].join("\n");
  console.log(report);
