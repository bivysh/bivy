...
| Variable | Type | Default | Notes |
| --- | --- | --- | --- |
| `EPHEMERAL_MACHINES_ENABLED` | `1` to enable; `0` to disable | UI/managed off | Single deployment flag for the UI and new cloud-machine launches. Managed compute additionally requires configured operator credentials and all policy/hardening prerequisites. `0` blocks new launches in both lanes, never cleanup. Unset preserves legacy BYO API behavior but does not opt into UI/managed compute. Runtime UI configuration works with the standard image, without rebuilding |
