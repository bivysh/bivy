...
| [Grok](agents/grok.md) | `grok` | Beta | Yes | Yes | Yes | No | Yes (`-m`) | Effect-level (FS/MCP/net) | No | Grok CLI/xAI | Official `grok -p` (install via `curl -fsSL https://x.ai/cli/install.sh \| bash`). SuperGrok/X subscription → `~/.grok/auth.json` materialization; API key → `XAI_API_KEY`/`GROK_API_KEY`. |
