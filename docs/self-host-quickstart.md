...
| `ENFORCE_ENTITLEMENTS` | `0` | Self-host default: there's no paid tier here, so nothing is gated — push notifications, ephemeral runners, etc. work for every signed-in account. **Note:** `docker-compose.yml` currently only forwards `ENFORCE_ENTITLEMENTS` into the **relay** container's environment, not the control plane's — harmless at the `0` default, but if you set it to `1` to gate the control plane too, add it to the `control-plane` service's `environment:` block yourself. |
...
| `POSTGRES_DB`, `POSTGRES_USER`, `POSTGRES_PASSWORD` | `bivy_control_plane` / `bivy` / random, `openssl rand -base64 32` | Postgres role and database. `docker-compose.yml` builds `DATABASE_URL` for the control plane from these three at container-start time — don't set `DATABASE_URL` directly in `deploy/.env`; it isn't read from there. |
...
| `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `STRIPE_PRICE_INDIVIDUAL`, `STRIPE_PRICE_TEAM` | Billing | `STRIPE_SECRET_KEY` and `STRIPE_WEBHOOK_SECRET` must be set together — the control plane refuses to start otherwise (an unverifiable webhook is worse than none). |
| `BILLING_SUCCESS_URL`, `BILLING_CANCEL_URL`, `BILLING_PORTAL_RETURN_URL` | Stripe checkout/portal redirects | Optional even with Stripe enabled — each falls back to a same-origin URL derived from the request. Not written by `deploy/self-host.sh`; add them only if you want a custom redirect target. |
