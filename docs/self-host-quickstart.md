...
| `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `STRIPE_PRICE_PRO`, `STRIPE_PRICE_TEAM` | Billing | `STRIPE_SECRET_KEY` and `STRIPE_WEBHOOK_SECRET` must be set together — the control plane refuses to start otherwise (an unverifiable webhook is worse than none). |
