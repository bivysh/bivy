...
| AWS EC2 | Broadest path to memory, CPU, GPU, networking and compliance | UI exposes only T3 up to 32 GB | Default-VPC assumption, no subnet/security group, tiny default root disk, no M/R/G families, no GPU image/drivers, cumbersome credential input | Highest-priority expansion |
