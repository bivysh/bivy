...
> **Node-less inheritance (hosted).** The peer-only limit is lifted for **hosted-provisioning** accounts, so a user with *no persistent node at all* (phone-only) can sign a subscription in once and have every future hosted ephemeral inherit it. The control plane escrows the **model-auth vault key** at rest (`hosted_model_auth_keys`, sealed with the per-account hosted key — same posture as provider tokens / the room key) and serves it (`GET /node/model-auth-vault` → `hostedKey`) only to hosted accounts' nodes; a hosted node writes it via `PUT /node/model-auth-key/hosted-escrow` (gated on `BIVY_GITHUB_HOSTED_TASKS`, and the CP re-checks the account is hosted). A lone hosted ephemeral with vault ciphertext but no peer-wrapped key adopts the escrowed key and decrypts the whole vault (incl. subscription OAuth). Non-hosted accounts stay fully peer-wrapped and CP-blind. Pairs with the **app-brokered login relay** (`provider.oauth.start` / `provider.oauth.code` → `startOAuthLogin` + the `oauthLogins` manual-code paste-back, `ProviderConnect.tsx` / `NodeReconnectSheet.tsx`): the login runs on the (ephemeral) node, the app shows the auth URL / device code, the user pastes the code back, and the completed token is pushed to the vault + escrowed above. Abandoned logins are swept (TTL + on-start) so a short-lived node doesn't leak login state or its callback server (`oauth-login-sweep.ts`). Covered by `test/hosted-model-auth-escrow.test.ts` and `oauth-login-sweep.test.ts`.
...
2. ✅ **A guaranteed pre-teardown snapshot flush.** `flushSessionSnapshots()` runs inside the daemon's ephemeral self-teardown (`evaluateEphemeralTeardown`) — **awaited before** `performSelfTeardown`, so the snapshot is persisted before the machine goes away (a `ephemeralTearingDown` latch prevents a racing re-entry). Reuses the Milestone-1 teardown path rather than the device timer.
