...
3. ◻ **Launch-with-existing node/session + restore bootstrap** — `launchEphemeralMachine` restore opt (reuse the old `sessionId`/`nodeId` + baked room key so the new machine can decrypt) + a `BIVY_RESTORE=<sessionId>` bootstrap export.
4. ◻ **A boot-time restore orchestrator** — on `BIVY_RESTORE`, GET the blob, `applySessionSnapshot` (rewrite EventLog + apply/ materialize the checkpoint — already unit-tested), then re-derive the runtime. `runtimeSessionRef` is **not self-sufficient** (for Claude Code it names an on-disk JSONL that won't exist on a fresh machine), so rebuild reconstructs it from the EventLog via `writeHistory`/`importHistoryForFork` or falls back to a seeded-summary continuation ("resumed/fresh"). Fidelity is "reconstructed", not byte-identical.
5. ◻ **A "Suspended/Archived" session row that re-provisions** on tap (mirroring the Sprites wake UX) — a sibling branch in `resumeAndConnectNode` for a destroy-lane node that's gone.

Status: the **capture half** (durable encrypted snapshot store + pre-teardown flush) is implemented and verified; the **restore half** (restore-mode boot orchestrator + runtime reconstruction + re-provision UI) is the remaining increment — its core `applySessionSnapshot` is already tested, so what's left is the boot/launch wiring, the runtime reconstruction, and the UI trigger. P2's device-vault key machinery is the shared prerequisite for making the blob readable on a *different* device (same-device re-provision needs only the baked room key).
