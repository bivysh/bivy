...
Implemented now: the remote UI's node selector has an **⚡ Ephemeral session** entry. Choosing it opens a provider picker (Fly.io, Hetzner Cloud, AWS EC2); picking a provider shows the inline quick guide below plus a token field. Saving stores the token **on the device** — in the PWA's IndexedDB (`bivy-ephemeral` DB, `provider-keys` store) via the shared `BivyEphemeralKeys` module. At rest the token never touches the control plane or any node, and it isn't tied to a node, so a user can add it from their phone before they own any computer/server (the first machine they run can itself be ephemeral). Machine provisioning and teardown, which consume this device-held token, are implemented — see "Provisioning path" below. (Note that on the transport currently in use, the token *is* seen in-flight by the control plane during each provider call; see that section for the trade-off and the end-to-end path that would remove it.)
...
Today there is one transport in use:

- **cloud relay (the only path the client uses):** the browser posts the request to the control plane's `POST /api/ephemeral/exec` (`requireUser`), which forwards it once to the allowlisted provider host and **never stores or logs it**. Every launch, status poll, and destroy goes this way, including a phone's very first machine. The trade-off is that the token is TLS-terminated in-flight at the control plane rather than staying strictly device-to-provider — a softer guarantee than the rest of Bivy's end-to-end model. `cloudExec` in `packages/core/src/ephemeral.ts` is the client half; it is what every call site in `packages/web/src/store/controller.ts` passes.

A second, end-to-end transport is **built but not yet wired up on the client:**

- **node-broker (present on the node, no client caller):** when an account has a node online, that node could run the provider request itself over the E2E relay, keeping the token off the control plane entirely. The node-side handler exists and is unit-tested — the `ephemeral.exec` frame in `src/server.ts` dispatches to `execEphemeralRequest` in `src/ephemeral-exec.ts` (`test/ephemeral-exec.test.ts`) — but nothing in the client emits that frame, so the path is dormant. There is no `nodeExec` in `packages/core` yet. Wiring the client to prefer it when a node is online is the natural next step; until then, treat the token as visible in-flight to the control plane for every ephemeral operation.

The SSRF host allowlist — `api.hetzner.cloud`, `api.machines.dev`, `api.fly.io`, plus per-region `ec2.<region>.amazonaws.com`/`ssm.<region>.amazonaws.com` hosts for AWS — is enforced by both proxy halves and kept in lock-step across three copies: `ALLOWED_HOSTS` in `packages/core/src/ephemeral.ts`, `EPHEMERAL_ALLOWED_HOSTS` in `src/ephemeral-exec.ts` (the dormant node broker), and the control plane's own copy in `services/control-plane/src/index.ts` (the active path) — since the control plane is deliberately kept dependency-light and doesn't import `@bivy/core`.

Launch flow (browser):

1. Enroll a fresh `nodeId` with the control plane (`POST /nodes/enroll`, account session, `ephemeral: true`) → enrollment token. Enrollment happens *before* the machine is provisioned, so it is also the point where a plan's concurrent-runner cap is enforced — an account already at its limit is refused here and no cloud resource is ever created. The `ephemeral` flag is recorded server-side (a `nodes.ephemeral` column) so runners are counted against `ephemeralConcurrent` rather than `maxNodes`; on the free plan that is one live runner at a time, with paid plans unlimited. See the entitlements notes in `services/control-plane/src/store.ts`.
2. Mint a 32-byte room key; keep it on the device (for reach) and bake it into the machine's `relay.json`. The room key is never sent to the relay or control plane on any path — the provider *token* is the only secret exposed in-flight, and only on the cloud-relay path currently in use.
