...
**Status: planned, not launchable.** The adapter is written against E2B's documented REST shape and unit-tested with an injected transport (`packages/core/test/ephemeral-e2b.test.ts`), but the catalog deliberately blocks credential validation and launch until the following are resolved:
