...
Capacity is never shared across customers. Normal TTL/provider teardown remains
the bill-safety backstop. A standby with less than five minutes remaining is deleted
...
1. **Catalog entry** — add an `EphemeralProviderCatalog` object to `EPHEMERAL_PROVIDERS` in `packages/core/src/ephemeral.ts`: `id`, `name`, `computeClass` (`byo-cloud` or `managed-compute`), `tokenLabel` (what the UI's single text field is asking for — one bearer token, or a composite like AWS's `accessKeyId:secretAccessKey`), `blurb`, `steps` (numbered setup copy), and `links` (opens in a new tab). This alone makes the provider selectable and show its guide — the UI (`packages/web/src/components/Ephemeral.tsx`) renders straight from the catalog. Managed sandbox services are intentionally not supported today; any future integration must retain Bivy's portable checkpoint format and pass the same teardown certification as VM providers.
...
The hosted first-run flow closes that gap with a managed **authentication
Machine** (`POST /account/onboarding/auth-runner`). It uses the ordinary managed
...
Fly Machines, Hetzner, and AWS do not preserve process memory after teardown. Bivy therefore treats encrypted snapshots and git checkpoints—not provider-native suspension—as the portable continuity boundary.
...
Rebuild continuity depends on encrypting session state so only the account can read it on replacement compute—the same key-availability problem as [Closing the cold-start gap](#closing-the-cold-start-gap-device-seeded-model-keys).

### Rebuild-resume for destroy-when-done providers
...
5. ~◻~ **Re-provision entry.** `controller.reprovisionEphemeral(nodeId, sessionId)` re-launches with the restore opts and reconnects. The node-switcher **UI affordance** ("Rebuild"/reopen) is the thin remaining piece.
