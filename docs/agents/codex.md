...
for your account (its `hello` handshake), defaulting to whichever model Codex
marks as default (currently GPT-5.6 Sol, the latest frontier agentic coding
model). The session-less placeholder shown before that handshake is the current
GPT-5.6 lineup — Sol, Terra, and Luna — under the `openai-codex` provider.
