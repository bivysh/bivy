...
import type { SessionLimit } from "../policy/session-limit.js";
...
export type SessionRecord = { id: string; session: RuntimeSession; runtimeId: string; credentialLabels?: Record<string, string>; sandbox?: SandboxTier; approvalMode?: ApprovalMode; automationRunId?: string; delegationDepth?: number; workspace: string; sessionFile?: string; agentServiceAddress?: string; lastActivity?: unknown; lastTouchedAt?: number; isWorking?: boolean; backgroundTaskCount?: number; workingStartedAt?: number; lastProgressAt?: number; lastStructuralProgressAt?: number; lastFailureAt?: number; turnWatchdog?: NodeJS.Timeout; turnTimeoutSignal?: Promise<void>; turnTimeoutResolve?: () => void; turnTimedOut?: boolean; turnAttention?: TurnAttention; abortRecovery?: Promise<void>; authRequiredSignaled?: boolean; naming?: boolean; namedFromFirstPrompt?: boolean; namingAttempts?: number; firstNamingPrompt?: string; worktree?: Worktree; source?: BivySessionSource; forkedFrom?: string; branchPushed?: boolean; branchPushing?: boolean; prUrl?: string; prs?: PrRef[]; prDetecting?: boolean; tuiTermId?: string; tuiRefreshing?: boolean; remoteActive?: boolean; ephemeral?: boolean; unsubscribe?: () => void; paused?: boolean; warning?: string; costUsd?: number; usage?: UsageSnapshot; githubIssueUrl?: string; mcpRestore?: () => void; harnessTurnReady?: Promise<void>; harnessTurnFinished?: boolean; /** This run's review card, once decided (see AppService.runEnded). */ runReview?: Promise<import("../apps/types.js").AppReview | undefined>; workspaceState?: SessionWorkspaceState; lastPrompt?: string; lastPromptOptions?: PromptOptions; reroute?: SessionRerouteController; limitHit?: SessionLimit; seenAttachmentHashes?: Set<string>;
  /** The Effective Session Contract resolved once at session creation (see
   *  src/session/session-contract.ts). Never live-recomputed — a stored
   *  contract reflects what launch actually got, not the runtime's current
   *  catalog entry, so it can't silently "improve" behind the user later. */
