...
export class BivyCredentialStore {
  ...
  constructor(
    ...
    plaintextDir?: string,
    private readonly sealer: Sealer = nodeSealer,
  ) {
  ...
  private readDocument(): CredentialVaultDocument {
    ...
    let parsed: unknown;
    try {
      parsed = JSON.parse(this.sealer.open(this.key(), raw.trim()));
    } catch {
  ...
  private writeDocument(document: CredentialVaultDocument): void {
    fs.mkdirSync(this.vaultDir, { recursive: true, mode: 0o700 });
    const ciphertext = this.sealer.seal(this.key(), JSON.stringify(document));
...
export function createCredentialVault(
  ...
  return new BivyCredentialStore(vaultDir, plaintextDir, sealer);
}
