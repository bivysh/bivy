...
assert.deepEqual(installCommand("https://app.bivy.sh", "wss://relay.bivy.sh"), {
  command: "curl -fsSL https://bivy.sh/install.sh | BIVY_CONTROL_PLANE_URL=https://app.bivy.sh BIVY_RELAY_URL=wss://relay.bivy.sh bash",
  hosted: true,
  authenticated: false,
});
...
assert.equal(
  installCommand("https://bivy.example.com/", "wss://relay.example.com/", "sess_abc").command,
